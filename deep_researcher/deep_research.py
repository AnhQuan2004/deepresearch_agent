...
class DeepResearcher:
    ...
    def __init__(
        ...
        self._last_partial_result = None  # Lưu kết quả tạm thời để xử lý timeout
    ...
    async def _build_report_plan(self, query: str) -> ReportPlan:
        ...
        # Lưu kết quả tạm thời sau khi có report plan
        try:
            sections = [f"## {section.title}\n\nKey question: {section.key_question}" for section in report_plan.report_outline]
            self._last_partial_result = f"# {report_plan.report_title}\n\n" + "\n\n".join(sections)
            if report_plan.background_context:
                self._last_partial_result += f"\n\n## Background Context\n\n{report_plan.background_context}"
        except Exception as e:
            self._log_message(f"Warning: Failed to save partial result after plan: {e}")
    ...
    async def _create_final_report(
        ...
        # Lưu partial result sau khi đã có các section draft
        try:
            partial_sections = [f"## {s.section_title}\n\n{s.section_content}" for s in report_draft.sections]
            self._last_partial_result = f"# {report_plan.report_title}\n\n" + "\n\n".join(partial_sections)
        except Exception as e:
            self._log_message(f"Warning: Failed to save partial result: {e}")
